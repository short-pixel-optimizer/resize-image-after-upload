...
class Epsilon_Smart_Notification {
	...
	private function check_plugin( $slug = '' ) {
		$arr = array(
			...
			'label'     => __( 'Install and Activate', 'resize-image-after-upload' ),
		);

		if ( $arr['installed'] ) {
			$arr['needs'] = 'activate';
			$arr['class'] = 'button button-primary';
			$arr['label'] = __( 'Activate now', 'resize-image-after-upload' );
		}

		if ( $arr['active'] ) {
			$arr['needs'] = 'deactivate';
			$arr['class'] = 'deactivate-now button';
			$arr['label'] = __( 'Deactivate now', 'resize-image-after-upload' );
		}
